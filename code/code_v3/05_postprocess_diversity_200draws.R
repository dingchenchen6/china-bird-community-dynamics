...
# Override for 200 draws run (MUST be after all source() calls)
# utils_paths.R and utils_diversity.R also source 00_config.R,
# which would overwrite PSI_MAX_DRAWS back to 400 if placed earlier.
PSI_MAX_DRAWS <<- 200L
...
# Ensure grid_cell type matches (trend_summary_df uses character from sites vector)
...
message(sprintf("[05] Temporal dynamics computed. Non-NA rates: %s",
  paste(temporal_metrics, "=",
        round(apply(!is.na(temporal_arr), 3, mean) * 100, 1),
        collapse = ", ")))
